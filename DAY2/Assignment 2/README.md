...
## 4-bit Comparator Evidence

The comparator testbench first runs 20 deterministic random tests and then checks all 256 possible pairs of 4-bit values. The simulation output prints decimal values for `A` and `B`, the expected relation (`A > B`, `A < B`, or `A == B`), and the three comparator outputs. All 276 tests complete successfully.
...
![4-bit comparator simulation output](Simulation_4bit_compartor.png)

### Additional Simulation View

![4-bit comparator additional simulation view](Simulation_4bit_compartor_1.png)
