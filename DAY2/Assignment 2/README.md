...
| `encoder_8_to_3_tb.v` | Testbench for the encoder. It uses named port mapping while instantiating the design and a `for` loop to drive all valid one-hot input combinations. It also tests the all-zero default input. |
...
After running the simulation, open `encoder_8_to_3_tb.vcd` in a waveform viewer. The waveform shows the input bus `in` changing through the valid one-hot combinations and the output bus `out` changing to the corresponding 3-bit encoded value.
