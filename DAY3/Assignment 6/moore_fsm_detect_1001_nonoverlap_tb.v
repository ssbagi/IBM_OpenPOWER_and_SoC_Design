...
module moore_fsm_detect_1001_nonoverlap_tb;
	...
	moore_fsm_detect_1001_nonoverlap dut (
		.clk(clk),
		.reset(reset),
		.data_in(data_in),
		.detected(detected)
	...
	task send_bit;
		...
		begin
			case (expected_state)
				...
				4: expected_state = bit_value ? 1 : 0;
	...
	initial begin
		$dumpfile("moore_fsm_detect_1001_nonoverlap.vcd");
		$dumpvars(0, moore_fsm_detect_1001_nonoverlap_tb);
		...
		if (error_count == 0)
			$display("MOORE NON-OVERLAP TEST PASSED: 263 directed/random bits checked");
		else
